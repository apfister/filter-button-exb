/** @jsx jsx */
import {
  React, LinkType, BaseWidget, AllWidgetProps, DataRecord, ExpressionPartType, ExpressionResolverErrorCode, appActions, LinkTo, LinkResult, classNames,
  RepeatedDataSource, IMExpression, IMDataSourceInfo, jsx, ExpressionResolverComponent, Immutable, IMState, AppMode, ThemeButtonType, css, IMIconProps, SerializedStyles,
  getAppStore, DataSourceManager
} from 'jimu-core';
...
import { JimuMapViewComponent, JimuLayerViewComponent, JimuLayerView, JimuMapView } from 'jimu-arcgis';
...
interface State {
  ...
  selectedLayerView: JimuLayerView
}
...
export default class Widget extends BaseWidget<AllWidgetProps<IMConfig> & ExtraProps, State>{
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      definitionExpression: this.props.config.definitionExpression || '1=1',
      selectedLayerView: this.props.config.jimuLayerViewInfo || null
    };
  ...
  executeDefintionExpression = (expression, layerId) => {
    console.log(expression);

    // const ds = DataSourceManager.getInstance().getDataSource(layerId);
    // ds.layer.definitionExpression = expression;

    this.state.selectedLayerView.view.layer.definitionExpression = expression;
  ...
  getLinkComponent = () => {
    ...
    if (config.styleConfig && config.styleConfig.customStyle) {
      ...
      if (config.styleConfig.useCustom) {
        ...
        if (this.props.active && this.props.appMode !== AppMode.Run) {
          ...
          customStyle = {
            style: widgetState.isConfiguringHover ? { ...style, ...hoverStyle } : style,
            hoverStyle
          };
        } else {
          customStyle = {
    ...
    if (linkParam && linkParam.linkType) {
      ...
      return <Link onClick={() => {
        this.executeDefintionExpression(this.props.config.definitionExpression, this.props.config.selectedDatasourceId);
      }} target={target} queryObject={queryObject}
        ...
        <div style={{ display: 'none' }}>
          {
            ...
            <div>
              <ExpressionResolverComponent useDataSources={this.props.useDataSources} expression={this.state.textExpression}
                records={this.getRecordsFromRepeatedDataSource()} onChange={this.onTextExpResolveChange}
              />
              <ExpressionResolverComponent useDataSources={this.props.useDataSources} expression={this.state.tipExpression}
                records={this.getRecordsFromRepeatedDataSource()} onChange={this.onTipExpResolveChange}
              />
              <ExpressionResolverComponent useDataSources={this.props.useDataSources} expression={this.state.urlExpression}
                records={this.getRecordsFromRepeatedDataSource()} onChange={this.onUrlExpResolveChange}
              />
            </div>
          }

          {this.props.config.hasOwnProperty("useMapWidgetIds") &&
          ...
          {this.props.config.hasOwnProperty("jimuLayerViewInfo") &&
            this.props.config.jimuLayerViewInfo && (
              <JimuLayerViewComponent
                ...
                onLayerViewCreated={(jmlv: JimuLayerView) => {
                  this.setState({ selectedLayerView: jmlv });
                }}
